...
- [x] **T7.1** — **Migrate from AngularJS to Vue 3** — Complete rewrite of the frontend from AngularJS 1.8.3 to Vue 3 (CDN-based, no build step). Replaced: `ChartController` → `ChartView` component with Composition API, `chartjsChart` directive → `ChartjsChart` component, `choroplethMap` directive → `ChoroplethMap` component, `dataService`/`dateFilterState`/`loading` factories → composables (`useChartData`, `useDateFilter`, `useLoading`). Removed jQuery, AngularJS, angular-route, angular-sanitize, angular-cookies, angular-resource, UI Bootstrap. Added Vue 3 + Vue Router 4 via CDN. Date pickers changed from UI Bootstrap popup to native `<input type="date">`. All 12 nav views preserved (10 demographics + 2 maps). *(completed)*
