angular.module('mturk').controller('BirthSurveyController', ['$scope', '$filter', 'dataService', 'chartUtils',
    ...
    $scope.load = function(){
        dataService.loadDemographicsSurvey($filter('date')($scope.from, 'MM/dd/yyyy'), 
            ...
            chartUtils.populate($scope.chart, response.labels.yearOfBirth, response.byBirthday);
        }, function(error){
            console.log(error);
        });
    };
