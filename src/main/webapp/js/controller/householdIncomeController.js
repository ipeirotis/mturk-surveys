angular.module('mturk').controller('HouseholdIncomeController', ['$scope', '$filter', 'dataService', 'chartUtils',
    ...
    $scope.load = function(){
        dataService.loadDemographicsSurvey($filter('date')($scope.from, 'MM/dd/yyyy'), 
            ...
            chartUtils.populate($scope.chart, response.labels.householdIncome, response.byHouseholdIncome);
        }, function(error){
            console.log(error);
        });
    };
