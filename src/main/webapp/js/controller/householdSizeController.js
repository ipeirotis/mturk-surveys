angular.module('mturk').controller('HouseholdSizeController', ['$scope', '$filter', 'dataService', 'chartUtils',
    ...
    $scope.load = function(){
        dataService.loadDemographicsSurvey($filter('date')($scope.from, 'MM/dd/yyyy'), 
            ...
            chartUtils.populate($scope.chart, response.labels.householdSize, response.byHouseholdSize);
        }, function(error){
            console.log(error);
        });
    };
