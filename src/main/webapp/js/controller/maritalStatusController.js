angular.module('mturk').controller('MaritalStatusController', ['$scope', '$filter', 'dataService', 'chartUtils',
    ...
    $scope.load = function(){
        dataService.loadDemographicsSurvey($filter('date')($scope.from, 'MM/dd/yyyy'), 
            ...
            chartUtils.populate($scope.chart, response.labels.maritalStatus, response.byMaritalStatus);
        }, function(error){
            console.log(error);
        });
    };
