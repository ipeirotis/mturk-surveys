angular.module('mturk').controller('GenderSurveyController', ['$scope', '$filter', 'dataService', 'chartUtils',
    ...
    $scope.load = function(){
        dataService.loadDemographicsSurvey($filter('date')($scope.from, 'MM/dd/yyyy'), 
            ...
            chartUtils.populate($scope.chart, response.labels.gender, response.byGender);
        }, function(error){
            console.log(error);
        });
    };
