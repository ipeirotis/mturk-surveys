/**
 * Choropleth Map Directive for AngularJS
 * Renders world or US state choropleths using D3.js v7 + TopoJSON
 */
(function (window, angular) {
    'use strict';

    // US state populations (2020 Census) for per-capita normalization
    var US_STATE_POP = {
    ...
    // FIPS code -> state abbreviation mapping for TopoJSON
    var FIPS_TO_STATE = {
    ...
    // Cache loaded TopoJSON so we don't re-fetch
    var topoCache = {};

    function loadTopo(url, callback) {
        if (topoCache[url]) {
            callback(topoCache[url]);
            return;
        }
    ...
    angular.module('mturk').directive('choroplethMap', ['$timeout', function ($timeout) {
        return {
            restrict: 'A',
            scope: {
                mapData: '=',
            ...
            link: function ($scope, $elm) {
                ...
                function render() {
                    ...
                    if (type === 'us') {
                        renderUsMap(el, data, $scope.normalized);
                    } else {
                ...
                function renderWorldMap(el, data) {
                    loadTopo(worldTopoUrl, function(world) {
                        ...
                        // Build ISO numeric -> count lookup
                        // The world-atlas uses numeric country IDs; we need a mapping
                ...
                function renderUsMap(el, data, normalized) {
                    loadTopo(usTopoUrl, function(us) {
                        ...
                        states.features.forEach(function(f) {
                            ...
                            if (normalized && count > 0) {
                                ...
                                val = (count / pop) * 1000000; // per million
                            }
                ...
                // ISO 3166-1 numeric -> alpha-2 mapping for world-atlas
