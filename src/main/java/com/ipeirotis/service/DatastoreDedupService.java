...
public class DatastoreDedupService {
	 ...
	 * Scans the entire Datastore using cursor-based pagination, groups by (workerId, hitId),
	 * and deletes all but the earliest entry per group. This catches cross-midnight duplicates
	 * that per-day dedup misses.
	 *
	 * @return number of duplicate entities deleted
	 */
	public int deduplicateGlobal() {
		Map<String, List<UserAnswer>> groups = new LinkedHashMap<>();
		int totalScanned = 0;

		// Paginate through all UserAnswer entities
		...
		for (List<UserAnswer> group : groups.values()) {
			...
			// Already ordered by date (from query), use ID as tiebreaker
			group.sort((a, b) -> {
				int dateCompare = compareDates(a.getDate(), b.getDate());
				if (dateCompare != 0) return dateCompare;
				return Long.compare(a.getId() != null ? a.getId() : 0, b.getId() != null ? b.getId() : 0);
		...
		if (toDelete.isEmpty()) {
			logger.info("Global dedup: scanned " + totalScanned + " entities, no duplicates found");
			return 0;
		}
		...
		logger.info("Global dedup: scanned " + totalScanned + " entities, deleted "
				+ toDelete.size() + " duplicates");
		return toDelete.size();
	}
